...
def compute_equilibrium_returns(returns, window=252, risk_free_rate=0.0):
    ...
    delta_df = pd.DataFrame(delta_history, index=dates, columns=["Implied_Risk_Aversion"])

    return pi_df, w_eq, delta_df 
...
def main():
    ...
    pi_df, w_eq,delta_df = compute_equilibrium_returns(returns)

    pi_df.to_csv(os.path.join(OUTPUT_DIR, "equilibrium_returns.csv"))
    delta_df.to_csv(os.path.join(OUTPUT_DIR, "implied_risk_aversion.csv"))
